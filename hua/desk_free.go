...
//'进入房间响应消息
func (t *Desk) freeEnterMsg(userid string) *pb.SJHFreeEnterRoom {
	msg := new(pb.SJHFreeEnterRoom)
	//房间数据
	msg.Roominfo = handler.PackJHFreeRoom(t.DeskData)
	msg.Roominfo.State = t.state
	//坐下玩家信息
	msg.Userinfo = t.freeSeatBetsMsg()
	//位置下注信息
	msg.Betsinfo = handler.PackJHRoomBets(t.DeskFree.SeatBets)
	return msg
}

//进入消息
func (t *Desk) freeCameinMsg(userid string) {
	msg := new(pb.SJHFreeCamein)
	msg.Userinfo = t.freeSeatRoleMsg(userid)
	t.broadcast(msg)
}

//位置上玩家数据
func (t *Desk) freeSeatRoleMsg(userid string) (msg *pb.JHFreeUser) {
	if v, ok := t.roles[userid]; ok {
		if v.Seat == 0 {
			return //没有坐下不广播
		}
		msg = handler.PackJHFreeUser(v.User)
		msg.Seat = v.Seat
		if t.DeskFree == nil {
			return
		}
...
//所有坐下玩家数据
func (t *Desk) freeSeatBetsMsg() (msg []*pb.JHFreeUser) {
...
//玩家下注数据
func (t *Desk) freeBetsMsg(userid string) (msg []*pb.JHRoomBets) {
	...
	case int32(pb.DEALER_DOWN):
		...
		case int32(pb.STATE_BET): //下注中
			if userid == t.DeskGame.Dealer {
				...
				msg := handler.BeJHDealerMsg(0, int64(num), t.DeskGame.Dealer,
...
//'获取上庄列表消息
func (t *Desk) dealerListMsg() (msg *pb.SJHFreeDealerList) {
	msg = new(pb.SJHFreeDealerList)
	if !t.isFree() {
...
func resFreeBet(seat, beseat uint32, val, coin,
	bets int64, userid string) *pb.SJHFreeBet {
	return &pb.SJHFreeBet{
		Seat:   seat,
		Beseat: beseat,
		Value:  uint32(val),
		Coin:   coin,
...
//' 百人输赢趋势
func (t *Desk) freeTrends() *pb.SJHFreeTrend {
	msg := new(pb.SJHFreeTrend)
...
//' 百人上局赢家列表
func (t *Desk) freeWiners() *pb.SJHFreeWiners {
	msg := new(pb.SJHFreeWiners)
...
//' 百人玩家列表
func (t *Desk) freeRoles() *pb.SJHFreeRoles {
	msg := new(pb.SJHFreeRoles)
...
//' 百人坐下
func (t *Desk) freeSit(userid string, arg *pb.CJHSit) (msg *pb.SJHSit) {
...
func (t *Desk) sitCheck(userid string, arg *pb.CJHSit) pb.ErrCode {
	user := t.getPlayer(userid)
	if user == nil {
		glog.Errorf("userid %s not exist", userid)
		return pb.NotInRoom
...
func (t *Desk) freeGameOver() {
	...
	//
	var a1 uint32 = algo.Hua(cs1) //1位置庄家牌力
	var a2 uint32 = algo.Hua(cs2) //2位置闲家牌力
	var a3 uint32 = algo.Hua(cs3) //3位置闲家牌力
	var a4 uint32 = algo.Hua(cs4) //4位置闲家牌力
	var a5 uint32 = algo.Hua(cs5) //5位置闲家牌力
...
//' 发牌消息
func resDraw(seat uint32, state int32, cards []uint32) *pb.SJHDraw {
	return &pb.SJHDraw{
		Seat:  seat,
		State: state,
		Cards: cards,
...
func resFreeStart(dealer, photo, nickname string, state int32,
	carry int64, dealerNum, left uint32) *pb.SJHFreeGamestart {
	return &pb.SJHFreeGamestart{
		Dealer:        dealer,
		Photo:         photo,
		State:         state,
		Coin:          carry,
...
//' 游戏结束消息
func (t *Desk) resOverFree() *pb.SJHFreeGameover {
	var left uint32
	if t.DeskGame.Dealer == "" {
		left = 1
	} else {
