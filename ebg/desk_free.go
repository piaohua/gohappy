...
//'进入房间响应消息
func (t *Desk) freeEnterMsg(userid string) *pb.SEBFreeEnterRoom {
	msg := new(pb.SEBFreeEnterRoom)
	//房间数据
	msg.Roominfo = handler.PackEBFreeRoom(t.DeskData)
	t.freeRoomDataMsg(msg.Roominfo)
	//坐下玩家信息
	msg.Userinfo = t.freeSeatBetsMsg()
	//位置下注信息
	msg.Betsinfo = handler.PackEBRoomBets(t.DeskFree.SeatBets)
	return msg
}

func (t *Desk) freeRoomDataMsg(msg *pb.EBFreeRoom) {
	msg.State = t.state
...
func (t *Desk) freeCameinMsg(userid string) {
	msg := new(pb.SEBFreeCamein)
	msg.Userinfo = t.freeSeatRoleMsg(userid)
	t.broadcast(msg)
}

//位置上玩家数据
func (t *Desk) freeSeatRoleMsg(userid string) (msg *pb.EBFreeUser) {
	if v, ok := t.roles[userid]; ok {
		if v.Seat == 0 {
			return //没有坐下不广播
		}
		msg = handler.PackEBFreeUser(v.User)
		msg.Seat = v.Seat
		if t.DeskFree == nil {
			return
		}
...
//所有坐下玩家数据
func (t *Desk) freeSeatBetsMsg() (msg []*pb.EBFreeUser) {
...
//玩家下注数据
func (t *Desk) freeBetsMsg(userid string) (msg []*pb.EBRoomBets) {
...
//'获取上庄列表消息
func (t *Desk) dealerListMsg() (msg *pb.SEBFreeDealerList) {
	msg = new(pb.SEBFreeDealerList)
	if !t.isFree() {
...
func resFreeBet(seat, beseat uint32, val, coin,
	bets int64, userid string) *pb.SEBFreeBet {
	return &pb.SEBFreeBet{
		Seat:   seat,
		Beseat: beseat,
		Value:  uint32(val),
		Coin:   coin,
...
//' 百人输赢趋势
func (t *Desk) freeTrends() *pb.SEBFreeTrend {
	msg := new(pb.SEBFreeTrend)
...
//' 百人上局赢家列表
func (t *Desk) freeWiners() *pb.SEBFreeWiners {
	msg := new(pb.SEBFreeWiners)
...
//' 百人玩家列表
func (t *Desk) freeRoles() *pb.SEBFreeRoles {
	msg := new(pb.SEBFreeRoles)
...
//' 百人坐下
func (t *Desk) freeSit(userid string, arg *pb.CEBSit) (msg *pb.SEBSit) {
...
func (t *Desk) sitCheck(userid string, arg *pb.CEBSit) pb.ErrCode {
	user := t.getPlayer(userid)
	if user == nil {
		glog.Errorf("userid %s not exist", userid)
		return pb.NotInRoom
...
func (t *Desk) shuffle() {
	...
	d := make([]uint32, algo.EbgNumCard, algo.EbgNumCard)
	copy(d, algo.EbgCARDS)
	//测试暂时去掉洗牌
	for i := range d {
		j := rand.Intn(i + 1)
		d[i], d[j] = d[j], d[i]
...
func (t *Desk) freeGameOver() {
	...
	//
	var a1 uint32 = algo.Ebg(t.DeskData.Mode, cs1) //1位置庄家牌力
	var a2 uint32 = algo.Ebg(t.DeskData.Mode, cs2) //2位置闲家牌力
	var a3 uint32 = algo.Ebg(t.DeskData.Mode, cs3) //3位置闲家牌力
	var a4 uint32 = algo.Ebg(t.DeskData.Mode, cs4) //4位置闲家牌力
	var a5 uint32 = algo.Ebg(t.DeskData.Mode, cs5) //5位置闲家牌力
...
func (t *Desk) setFreeRecord() {
	for k, v := range t.DeskFree.Score2 {
		...
		msg3 := handler.TaskUpdateMsg(1, pb.TASK_TYPE30, k)
		pid.Tell(msg3)
		msg4 := handler.TaskUpdateMsg(1, pb.TASK_TYPE31, k)
		pid.Tell(msg4)
	}
}
...
//' 发牌消息
func resDraw(seat uint32, state int32, cards []uint32) *pb.SEBDraw {
	return &pb.SEBDraw{
		Seat:  seat,
		State: state,
		Cards: cards,
...
func resFreeStart(dealer, photo, nickname string, state int32,
	carry int64, dealerNum, left uint32) *pb.SEBFreeGamestart {
	return &pb.SEBFreeGamestart{
		Dealer:        dealer,
		Photo:         photo,
		State:         state,
		Coin:          carry,
...
//' 游戏结束消息
func (t *Desk) resOverFree() *pb.SEBFreeGameover {
	var left uint32
	if t.DeskGame.Dealer == "" {
		left = 1
	} else {
...
func (t *Desk) taskHandler() {
	for k, v := range t.DeskFree.Power {
		...
		switch v {
		case algo.EBG10:
			taskType = pb.TASK_TYPE22
		case algo.EBGDui:
			taskType = pb.TASK_TYPE23
		case algo.BAIBAN:
			taskType = pb.TASK_TYPE24
		}
		if int32(taskType) == 0 {
			continue
		}
