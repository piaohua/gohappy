...
//'进入房间响应消息
func (t *Desk) privEnterMsg(userid string) *pb.SNNEnterRoom {
	msg := new(pb.SNNEnterRoom)
	//房间数据
...
//投票信息
func (t *Desk) voteInfoMsg() (msg *pb.NNRoomVote) {
	msg = new(pb.NNRoomVote)
	if t.DeskPriv != nil {
...
//语音房间投票信息
func (t *Desk) voiceInfoMsg() (msg *pb.NNRoomVoice) {
	msg = new(pb.NNRoomVoice)
	if t.DeskGame != nil {
...
//发起投票
func (t *Desk) launchVote(userid string, vote uint32) (msg *pb.SNNLaunchVote) {
	msg = new(pb.SNNLaunchVote)
	errcode := t.checkVote()
	if errcode != pb.OK {
		msg.Error = errcode
		return
...
//投票
func (t *Desk) privVote(userid string, vote uint32) (msg *pb.SNNVote) {
	msg = new(pb.SNNVote)
	errcode := t.checkVote()
	if errcode != pb.OK {
		msg.Error = errcode
...
func (t *Desk) setRecord(score map[uint32]int64) {
	for k, v := range score {
		...
		case int32(pb.ROOM_TYPE0): //自由
			...
			msg3 := handler.TaskUpdateMsg(1, pb.TASK_TYPE25, user.GetUserid())
			pid.Tell(msg3)
			msg4 := handler.TaskUpdateMsg(1, pb.TASK_TYPE31, user.GetUserid())
			pid.Tell(msg4)
		case int32(pb.ROOM_TYPE1): //私人
			if t.DeskData.Pub {
