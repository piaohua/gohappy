...
//'玩家准备
func (t *Desk) readying(userid string, ready bool) (msg *pb.SSGReady) {
	msg = new(pb.SSGReady)
	if t.isFree() {
...
func (t *Desk) choiceDealer(userid string, isDealer bool,
	num uint32) (msg *pb.SSGDealer) {
	msg = new(pb.SSGDealer)
	if t.isFree() {
		msg.Error = pb.OperateError
		return
	}
...
func (t *Desk) choiceNiu(userid string, num uint32,
	cs []uint32, ctx actor.Context) (msg *pb.SSGiu) {
	msg = new(pb.SSGiu)
	if t.isFree() {
		msg.Error = pb.OperateError
		return
